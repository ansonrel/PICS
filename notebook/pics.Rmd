...
do_pics_comb_plot <- function(dat, ks_pos = FALSE, ks_neg = FALSE, digits = 0){
  # for each dataset
  ...
  for (i in names(dat)){
    for (j in neg[[i]]){
      for (k in pos[[i]]){
        ...
        pics <- lapply(norm_nam, function(x) round(picsScore(datsub, x, ks_pos = ks_pos, ks_neg = ks_neg), digits))
        names(pics) <- norm_nam
...
```{r warning=FALSE, message=FALSE}
out <- do_pics_comb_plot(dat, ks_pos = TRUE, ks_neg = TRUE, digits = 2)
```
  ...
  for (i in names(dat)){
    for (j in neg[[i]]){
      for (k in pos[[i]]){
        ...
        datsub <- dat[[i]][c(j, k)]
        picspos <- round(picsScore(datsub, ASSAY, ks_pos = TRUE, ks_neg = TRUE, positive_only = TRUE), 3)
        picsneg <- round(picsScore(datsub, ASSAY, ks_pos = TRUE, ks_neg = TRUE, negative_only = TRUE), 3)
        out_scores[[ASSAY]][[paste(i, j, k, sep = ",")]] <- list(picspos, picsneg)
  ...
  for (j in neg[[i]]){
    for (k in pos[[i]]){
      ...
      for (l in norm_nam){
              ...
              picspos <- round(picsScore(datsub, l, ks_pos = TRUE, ks_neg = TRUE, positive_only = TRUE), 3)
              picsneg <- round(picsScore(datsub, l, ks_pos = TRUE, ks_neg = FALSE, negative_only = TRUE), 3)
              picsglob <- round(picsScore(datsub, l, ks_pos = TRUE, ks_neg = FALSE), 3)
...
AUC scores for positive vs negative markers: [NEW]
...
AUC vs PICS (using KS components) [NEW]
...
CD45RA should be highly expressed in naive T cells --> check the AUC for these [NEW]
...
CD45-1, which didn't perform good with AUC: [NEW]
