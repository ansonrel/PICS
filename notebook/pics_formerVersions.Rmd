...
This markdown summarizes the initial thought in the development of the PICS score: 
use t-test, sd, median, etc. and we found out it would be more suited to use the KS test for both negative and positive component. 
...
dat <- dat[c(grep("hao|kotliarov", names(dat)))]
...
abs_metadata <- rbind(abs_kot, abs_hao)
head(abs_metadata)
```
...
The original idea was to use t-test (positive component) and median*sd (negative component)

```{r warning=FALSE, message=FALSE}
out <- do_pics_comb_plot(dat, ks_pos = FALSE, neg_fun="med_sd", digits = 0)
```
...
**==> PICS neg using ks is not very good at differentiating signals** 
...
### PICS score with KS stat for positive & **negative components, weighted by sd**
  ...
  for (i in names(dat)){
    for (j in neg[[i]]){
      for (k in pos[[i]]){
        ...
        datsub <- dat[[i]][c(j, k)]
        picspos <- round(picsScore(datsub, ASSAY, ks_pos = TRUE, neg_fun = "KS_sd", positive_only = TRUE), 3)
        picsneg <- round(picsScore(datsub, ASSAY, ks_pos = TRUE, neg_fun = "KS_sd", negative_only = TRUE), 3)
        out_scores[[ASSAY]][[paste(i, j, k, sep = ",")]] <- list(picspos, picsneg)
...
ggplot(pics_melt, aes(x=PICS.pos, y=PICS.neg, col = normalization)) + 
geom_point() + geom_smooth(se = FALSE) + ggtitle("Positive vs Negative PICS score component\nKS test in positive & negative components (weighted by SD)") + 
  xlab("PICS positive component") + ylab("PICS negative component")
```
